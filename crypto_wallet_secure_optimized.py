...
class 安全工具:
    ...
    def 安全清除内存(数据):
        ...
        try:
            ...
            if isinstance(数据, str):
                # 字符串类型，先覆盖为随机字符，再覆盖为零
                长度 = len(数据)
                # 使用ctypes直接操作内存
                ctypes.memset(id(数据) + 20, ord('X'), 长度)
                ctypes.memset(id(数据) + 20, 0, 长度)
            elif isinstance(数据, bytes) or isinstance(数据, bytearray):
                # 字节类型，先覆盖为随机字节，再覆盖为零
                长度 = len(数据)
                if 长度 > 0:
                    # 使用ctypes直接操作内存
                    ctypes.memset(id(数据) + 33, 0xFF, 长度)
                    ctypes.memset(id(数据) + 33, 0, 长度)
                ...
                for key in 数据:
                    ...
                    数据[key] = None
                数据.clear()
            elif isinstance(数据, int) or isinstance(数据, float):
                # 数值类型，无法直接清除，但可以尝试覆盖
                pass  # Python中的数值是不可变的，无法直接清除
            
            # 强制垃圾回收
            安全工具.强制垃圾回收()
        except Exception as e:
            # 捕获所有异常，确保不会因为清除失败而中断程序
            print(f"内存清除异常: {str(e)}")
            # 即使出错，也尝试进行垃圾回收
            安全工具.强制垃圾回收()
    ...
    def 检查系统安全性() -> Dict[str, bool]:
        ...
        try:
            ...
            # 尝试连接多个网站，增加检测可靠性
            for host in ["www.baidu.com", "www.google.com", "www.cloudflare.com"]:
                try:
...
class 熵源生成器:
    ...
    def 收集所有可用熵(self, 包含用户熵: bool = True) -> None:
        ...
        # 显示初始熵池状态
        熵池状态 = self.获取熵池状态()
        print(f"初始熵池状态: {熵池状态['熵池健康度']}% ({熵池状态['熵源数量']}/{ENTROPY_SOURCES_REQUIRED}个熵源)")
        if 熵池状态['熵源数量'] > 0:
        ...
        # 再次显示熵池状态
        熵池状态 = self.获取熵池状态()
        print(f"\n当前熵池状态: {熵池状态['熵池健康度']}% ({熵池状态['熵源数量']}/{ENTROPY_SOURCES_REQUIRED}个熵源)")
        print(f"已添加熵源: {', '.join(熵池状态['已添加熵源'])}")
...
class 钱包生成器:
    ...
    def 生成助记词(self, 强度: int = DEFAULT_ENTROPY_BITS) -> str:
        ...
        print(f"\n最终熵池状态: {熵池状态['熵池健康度']}% ({熵池状态['熵源数量']}/{ENTROPY_SOURCES_REQUIRED}个熵源)")
        print(f"已添加熵源: {', '.join(熵池状态['已添加熵源'])}")
...
def 显示安全提示(提示类型: str) -> None:
    ...
    提示 = {
        ...
        "密码短语": [
            "⚠️ 密码短语(passphrase)是BIP-39的可选\033[1;31m第25个词\033[0m",
            "✓ 增加了安全性，但也增加了丢失风险",
...
def 生成新钱包() -> None:
    ...
    try:
        ...
        print("\n===== 生成的助记词 =====\n")
        print("\033[1;32m" + "=" * 60 + "\033[0m")  # 绿色分隔线
        
        # 将助记词分成每行3个词
        助记词列表 = 助记词.split()
        每行词数 = 3
        行数 = (len(助记词列表) + 每行词数 - 1) // 每行词数  # 向上取整
        
        # 计算最长词的长度，用于对齐
        最长词长度 = max(len(词) for 词 in 助记词列表)
        
        for i in range(行数):
            开始索引 = i * 每行词数
            结束索引 = min((i + 1) * 每行词数, len(助记词列表))
            当前行词语 = 助记词列表[开始索引:结束索引]
            
            # 构建带序号和对齐的行
            行文本 = ""
            for j, 词 in enumerate(当前行词语):
                序号 = 开始索引 + j + 1
                # 使用左对齐确保词语对齐
                行文本 += f"{序号:2d}. {词:<{最长词长度}}  "
            
            # 使用黄色加粗显示
            print("\033[1;33m" + 行文本.rstrip() + "\033[0m")
        
        print("\033[1;32m" + "=" * 60 + "\033[0m")  # 绿色分隔线
        print(f"\n语言: {DEFAULT_LANGUAGE} (英文)")
        print(f"词数: {len(助记词列表)}")
        ...
        for i in range(组数):
            ...
            while True:
                try:
                    ...
                    # 检查阈值是否为1且成员数大于1，这种情况会导致错误
                    if 阈值 == 1 and 成员数 > 1:
                        print("错误: 当成员数大于1时，阈值不能为1。")
                        print("这是因为SLIP-39规范不允许在成员数大于1时设置阈值为1。")
                        print("请选择更高的阈值，或者将成员数也设为1（1-of-1分享）。")
                        continue
                    if 1 <= 阈值 <= 成员数:
                        每组阈值.append(阈值)
                        break
